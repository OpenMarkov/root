...
% ===================================================================
\section{Catálogo de problemas de diseño}
\label{sec:catalogo}

Esta sección proporciona una visión panorámica de los seis problemas de diseño
identificados en la jerarquía de \code{Potential}.  Cada problema se describe
de forma sintética aquí; las secciones~\ref{sec:prob1}--\ref{sec:prob6}
desarrollan el diagnóstico en profundidad.

La figura~\ref{fig:problemas} muestra la jerarquía actual con los seis problemas
localizados sobre las clases afectadas.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-problemas}
\caption{Vista panorámica de los seis problemas de diseño en la jerarquía de
         \code{Potential}.  Los colores indican la gravedad: rojo para clases
         con problemas directos, amarillo para clases afectadas.}
\label{fig:problemas}
\end{figure}
...
\begin{tabular}{clp{5.5cm}p{3.5cm}l}
\toprule
\textbf{Id} & \textbf{Problema} & \textbf{Descripción} & \textbf{Clase(s) afectada(s)} & \textbf{Principio} \\
\midrule
\rowcolor{lightgray}
P1 & \emph{God Class} con campos opcionales &
  \code{TablePotential} mezcla valores, estrategias de intervención e incertidumbre
  en una sola clase mediante campos que son \code{null} en >95\,\% de las instancias.
  El constructor de copia produce \emph{alias mutables} compartidos. &
  \code{TablePotential} (1.413~líneas) &
  SRP \\

P2 & Métodos abstractos forzados &
  \code{Potential} declara métodos abstractos (\code{reorder}, \code{project},
  \code{scalePotential}) que muchas subclases implementan devolviendo \code{null},
  lanzando excepciones o dejando el cuerpo vacío. &
  \code{UniformPotential}, \code{DeltaPotential}, \code{SumPotential}, \code{ProductPotential}, etc. &
  LSP, ISP \\

\rowcolor{lightgray}
P3 & \code{GTablePotential} hereda estado inútil &
  Extiende \code{TablePotential} pero nunca usa el array \code{values[]} heredado
  (desperdicio de \code{tableSize * 8} bytes).  Pasa \code{null} como
  \code{PotentialRole} al constructor del padre. &
  \code{GTablePotential<E>} &
  LSP \\

P4 & \emph{Downcasts} desprotegidos en inferencia &
  El módulo de inferencia trabaja directamente con \code{TablePotential}, no con
  \code{Potential} polimórficamente.  Los \emph{downcasts} desprotegidos
  (\code{(TablePotential) potential}) producen \code{ClassCastException} en
  tiempo de ejecución. &
  \code{VariableEliminationCore}, \code{VEPropagation}, etc. &
  LSP, OCP \\

\rowcolor{lightgray}
P5 & Dependencia cíclica &
  \code{TablePotential} $\to$ \code{StrategyTree} $\to$ \code{TreeADDPotential}
  $\to$ \code{TablePotential}.  Hace imposible razonar sobre cualquiera de las
  tres clases de forma aislada y bloquea la refactorización incremental. &
  \code{TablePotential}, \code{StrategyTree}, \code{TreeADDPotential} &
  Acoplamiento \\

P6 & \code{PotentialRole} como \emph{type code} &
  El enumerado \code{PotentialRole} controla el comportamiento de
  \code{TablePotential} mediante sentencias \code{switch}: \code{setUniform()},
  \code{normalize()}, \code{getInducedFindings()}.  Añadir un nuevo rol exige
  rastrear y actualizar todos los \code{switch}. &
  \code{TablePotential}, \code{DiscretePotentialOperations} &
  OCP \\
\bottomrule
\end{tabular}
\caption{Catálogo de los seis problemas de diseño identificados.  Los principios
         violados se refieren a los principios SOLID descritos en la
         sección~\ref{sec:fundamentos}.}
\label{tab:catalogo-problemas}
\end{table}

\subsection{Interdependencias entre problemas}

Los problemas no son independientes.  Resolverlos de forma aislada puede ser
insuficiente o incluso contraproducente:

\begin{itemize}
  \item P1 (God Class) y P6 (type code) están entrelazados: los \code{switch}
        sobre \code{PotentialRole} son parte de la lógica condicional que
        \code{TablePotential} acumula.  Resolver P1 sin abordar P6 simplifica
        los campos pero mantiene la dispersión de comportamiento.
  \item P2 (métodos forzados) es consecuencia directa de P4 (downcasts): el
        módulo de inferencia necesita que todos los potenciales tengan los
        mismos métodos porque no trabaja polimórficamente.  Resolver P2 sin
        abordar P4 es superficial: las subclases dejarían de tener métodos
        vacíos, pero la inferencia seguiría haciendo \emph{downcasts}.
  \item P3 (GTablePotential) es un efecto colateral de P1: si \code{TablePotential}
        no hubiera acumulado tantas responsabilidades, \code{GTablePotential}
        nunca habría necesitado heredar de ella para acceder a la lógica de
        indexación.
  \item P5 (dependencia cíclica) está ligado a P1: el ciclo existe porque
        \code{strategyTrees[]} vive dentro de \code{TablePotential}.  Extraer
        las estrategias a una subclase o a un contenedor externo rompe el ciclo.
\end{itemize}

Estas interdependencias explican por qué el documento propone opciones de
rediseño de distinto alcance (sección~\ref{sec:op1} a~\ref{sec:op5}) y por qué
se recomienda un enfoque incremental que aborde primero los problemas más
aislados (P1, P3, P5) antes de los más transversales (P2, P4, P6).
...
La figura~\ref{fig:op2-roles} muestra la estructura resultante.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-op2-roles}
\caption{Opción~2: Jerarquía basada en roles.  Cada valor del antiguo enum
         \code{PotentialRole} se convierte en una subclase de
         \code{DiscretePotential} con su propio comportamiento
         (\code{setUniform()}, \code{normalize()}, etc.).  Las operaciones de
         \code{DiscretePotentialOperations} reciben tipos precisos.}
\label{fig:op2-roles}
\end{figure}
...
La figura~\ref{fig:op4-visitor} muestra la separación entre el paquete de datos
y el paquete de visitantes.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-op4-visitor}
\caption{Opción~4: Patrón Visitor.  Las clases de datos (izquierda) solo
         implementan \code{accept()}.  Toda la lógica de operación reside en
         los visitantes concretos (derecha).  La doble indirección
         \code{accept}$\to$\code{visit} garantiza el despacho correcto sin
         \emph{downcasts}, pero obliga a actualizar todos los visitantes
         cada vez que se añade un nuevo tipo de potencial.}
\label{fig:op4-visitor}
\end{figure}
...
La figura~\ref{fig:op5-immutable} muestra la estructura resultante.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig-op5-immutable}
\caption{Opción~5: Potenciales inmutables con Builder.  Todos los campos de
         \code{TablePotential} son \code{final}; el constructor es privado y
         solo accesible a través del Builder.  Las operaciones devuelven nuevas
         instancias en lugar de mutar el objeto existente.  El recuadro inferior
         contrasta la API mutable actual con la API inmutable propuesta.}
\label{fig:op5-immutable}
\end{figure}

La construcción se realiza exclusivamente a través de un \emph{builder}:
...
% ===================================================================
\newpage
\part{Diseño ideal desde cero}
\label{part:diseno-ideal}

% ===================================================================
\section{Motivación}
\label{sec:ideal-motivacion}

Las cinco opciones de rediseño analizadas en la Parte~I parten de la
restricción de mantener la compatibilidad con el código existente.  Esa
restricción es sensata para una migración real, pero limita la exploración
del espacio de diseño: las decisiones históricas condicionan cada opción.

Esta parte elimina esa restricción y plantea la pregunta:
\emph{¿cómo diseñaríamos el sistema de potenciales si pudiéramos empezar
de cero, conociendo todas las necesidades actuales de OpenMarkov?}

El objetivo no es proponer una reescritura total ---sería impracticable---
sino identificar el diseño \emph{diana} hacia el que las refactorizaciones
incrementales deberían converger.  Cada decisión de diseño en una
refactorización concreta puede evaluarse contra este diseño ideal:
¿nos acerca a él o nos aleja?

\section{Requisitos del sistema de potenciales}
\label{sec:ideal-requisitos}

Para diseñar desde cero es necesario listar explícitamente los requisitos
que el sistema de potenciales debe satisfacer.  Estos requisitos provienen
del análisis del código actual, de las necesidades de los algoritmos de
inferencia, y de los casos de uso soportados por OpenMarkov.

\subsection{Requisitos funcionales}

\begin{enumerate}[label=\textbf{RF\arabic*}]
  \item \textbf{Representación polimorfa.}
        El sistema debe soportar múltiples formas de definir un potencial:
        tablas explícitas de probabilidades o utilidades, distribuciones
        uniformes, deterministas (delta), modelos canónicos (noisy-OR,
        noisy-MAX), árboles de decisión algebraicos (ADD), gaussianas
        condicionales, modelos lineales generalizados (GLM), funciones
        simbólicas, y potenciales de producto/suma para supervalores en
        diagramas de influencia.
        \label{req:rf1}

  \item \textbf{Proyección a tabla discreta.}
        Toda representación debe poder convertirse en una tabla discreta
        de valores \code{double[]} dada una evidencia, porque los algoritmos
        de inferencia exacta (eliminación de variables, propagación Hugin)
        operan exclusivamente sobre tablas discretas.  La proyección es
        la operación que convierte un potencial arbitrario en la
        representación de trabajo del motor de inferencia.
        \label{req:rf2}

  \item \textbf{Operaciones de inferencia sobre tablas.}
        Sobre las tablas discretas se deben soportar al menos: multiplicación,
        suma, marginalización (suma sobre un subconjunto de variables),
        multiplicación-y-marginalización combinada, normalización (condicional
        y conjunta), división, maximización (para resolución de decisiones),
        y reordenamiento de variables.
        \label{req:rf3}

  \item \textbf{Distinción semántica por rol.}
        Las tablas discretas tienen semánticas distintas según su rol:
        probabilidad condicional, probabilidad conjunta, utilidad, política
        de decisión, restricción de enlace.  Cada rol implica comportamiento
        diferente en la inicialización (\code{setUniform}), la normalización,
        y la inducción de hallazgos (\emph{induced findings}).
        \label{req:rf4}

  \item \textbf{Análisis coste-efectividad.}
        El sistema debe soportar tablas genéricas \code{GenericTable<E>} cuyos
        elementos no son \code{double} sino objetos compuestos (particiones
        coste-efectividad, \code{CEP}).  Estas tablas comparten la lógica de
        indexación (dimensiones, offsets) con las tablas discretas pero tienen
        operaciones propias (álgebra CE).
        \label{req:rf5}

  \item \textbf{Metadatos de inferencia.}
        Los resultados de la resolución de diagramas de influencia pueden
        incluir \emph{árboles de estrategia} (\code{StrategyTree[]}) que
        codifican la política óptima de intervención.  Los análisis de
        sensibilidad pueden incluir distribuciones de incertidumbre
        (\code{UncertainValue[]}) sobre cada valor de la tabla.  Ambos
        son ortogonales al dato principal (\code{double[]}) y a la
        semántica de rol.
        \label{req:rf6}

  \item \textbf{Redes temporales.}
        Potenciales como \code{SameAsPrevious} y \code{CycleLengthShift}
        representan relaciones temporales entre rodajas de una red dinámica.
        Deben poder participar en la proyección usando el contexto de las
        tablas ya proyectadas.
        \label{req:rf7}
\end{enumerate}
...
\begin{enumerate}[label=\textbf{RNF\arabic*}]
  \item \textbf{Rendimiento en inferencia.}
        Las operaciones sobre tablas discretas (especialmente
        \code{multiplyAndMarginalize}) son el cuello de botella de la
        inferencia.  El diseño no debe introducir indirecciones, copias
        defensivas ni creación de objetos innecesarios en el camino
        crítico.  El acceso directo al array \code{double[]} desde los
        algoritmos de operación debe ser posible sin penalización.
        \label{req:rnf1}

  \item \textbf{Extensibilidad: nuevos tipos de potencial.}
        Añadir un nuevo tipo de representación (p.~ej.\ una distribución
        de Weibull, un modelo de regresión logística) no debe requerir
        modificar las clases existentes ni los algoritmos de inferencia:
        basta con implementar la interfaz de proyección.
        \label{req:rnf2}

  \item \textbf{Extensibilidad: nuevas operaciones.}
        Añadir una nueva operación sobre tablas (p.~ej.\ serialización
        binaria, evaluación en GPU) no debe requerir modificar la jerarquía
        de potenciales.
        \label{req:rnf3}

  \item \textbf{Testabilidad.}
        Cada componente debe ser testable de forma aislada: las
        representaciones, las proyecciones, las operaciones sobre tablas,
        y los metadatos.  No deben existir dependencias cíclicas que
        impidan construir un componente sin los demás.
        \label{req:rnf4}

  \item \textbf{Seguridad ante errores de tipo.}
        El sistema de tipos de Java debe impedir, en la medida de lo posible,
        los errores que actualmente se manifiestan como \code{ClassCastException}
        o \code{NullPointerException} en tiempo de ejecución.  Si una operación
        no tiene sentido para un tipo de potencial, el compilador debe
        rechazarla.
        \label{req:rnf5}
\end{enumerate}

% ===================================================================
\section{Arquitectura propuesta: tres capas}
\label{sec:ideal-arquitectura}

El diseño propuesto se organiza en tres capas con responsabilidades claras.
La figura~\ref{fig:diseno-ideal} muestra la arquitectura completa.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-diseno-ideal}
\caption{Arquitectura ideal en tres capas.  La capa de representación modela
         la diversidad de potenciales.  La capa de proyección los convierte
         a tablas discretas.  La capa de computación opera uniformemente sobre
         tablas.  Los metadatos (estrategias, incertidumbre) se componen
         externamente.  El rol semántico (\code{PotentialSemantics}) encapsula
         el comportamiento variable de cada rol como un objeto con métodos
         propios, eliminando los \code{switch} sobre el enum.}
\label{fig:diseno-ideal}
\end{figure}

\subsection{Capa 1: Representación}
\label{sec:ideal-capa1}

La capa de representación modela la diversidad de formas en que el usuario
o el fichero de entrada puede definir un potencial.  Su interfaz raíz es:

\begin{lstlisting}[caption={Interfaz raíz de la capa de representación}]
public sealed interface Potential
    permits DiscreteTable, UniformPotential, DeltaPotential,
            ICIPotential, TreeADDPotential, GaussianPotential,
            GLMPotential, SumPotential, ProductPotential,
            SameAsPrevious, CycleLengthShift {

    List<Variable> getVariables();
    PotentialSemantics getSemantics();
    Potential deepCopy(ProbNet net);
}
\end{lstlisting}

\noindent \textbf{Decisiones clave:}

\begin{itemize}
  \item \code{Potential} es una \code{sealed interface} (Java~17+), no una
        clase abstracta.  Esto aporta dos ventajas:
        \begin{enumerate}
          \item El compilador conoce todos los subtipos en tiempo de
                compilación, lo que habilita \emph{pattern matching} exhaustivo
                en sentencias \code{switch} (Java~21+):
\begin{lstlisting}
switch (potential) {
    case DiscreteTable dt  -> projectTable(dt, evidence);
    case UniformPotential u -> projectUniform(u, evidence);
    case DeltaPotential d   -> projectDelta(d, evidence);
    // ... el compilador rechaza si falta un caso
}
\end{lstlisting}
          \item Impide que módulos externos creen subclases no previstas,
                manteniendo el control del polimorfismo.
        \end{enumerate}

  \item \code{Potential} \textbf{no declara \code{tableProject()},
        \code{reorder()} ni \code{scalePotential()}} como métodos
        abstractos.  Estas operaciones no son parte del contrato mínimo
        de todos los potenciales: \code{reorder()} no tiene sentido para
        \code{UniformPotential}; \code{scalePotential()} no tiene sentido
        para \code{DeltaPotential}.  En su lugar, las operaciones se
        modelan como \emph{interfaces de capacidad} (requisitos~\ref{req:rnf2}
        y~\ref{req:rnf5}).

  \item Cada tipo concreto implementa exactamente las interfaces de capacidad
        que soporta:
        \begin{itemize}
          \item \code{DiscreteTable} implementa \code{Projectable},
                \code{Reorderable}, \code{Scalable}.
          \item \code{UniformPotential} implementa \code{Projectable} solamente.
          \item \code{DeltaPotential} implementa \code{Projectable} y
                \code{Scalable}.
        \end{itemize}
        Si un algoritmo necesita reordenar un potencial, declara el tipo
        \code{Reorderable} en su firma; el compilador rechaza en tiempo de
        compilación cualquier potencial que no soporte esa operación.
\end{itemize}

\subsection{Capa 2: Proyección}
\label{sec:ideal-capa2}

La proyección es el puente entre la diversidad de representaciones y la
uniformidad que los algoritmos de inferencia necesitan.  Su contrato es
simple: dado un potencial arbitrario y un caso de evidencia, produce
una \code{DiscreteTable}.

\begin{lstlisting}[caption={Interfaz de proyección y contexto}]
public interface Projectable {
    DiscreteTable projectToTable(EvidenceCase evidence,
                                 ProjectionContext context);
}

public final class ProjectionContext {
    private final List<DiscreteTable> alreadyProjected;

    public DiscreteTable project(Potential potential) {
        if (potential instanceof Projectable p) {
            return p.projectToTable(evidence, this);
        }
        throw new NonProjectablePotentialException(potential);
    }
}
\end{lstlisting}
...
\begin{itemize}
  \item La proyección es \textbf{la única operación polimórfica} del sistema.
        Cada tipo de potencial sabe cómo convertirse a sí mismo en una tabla
        discreta; los algoritmos de inferencia no necesitan conocer los
        detalles internos de ninguna representación.

  \item El \code{ProjectionContext} proporciona acceso a las tablas ya
        proyectadas, necesario para tipos como \code{SameAsPrevious} que
        referencian resultados anteriores (requisito~\ref{req:rf7}).

  \item Después de la fase de proyección, toda la diversidad de representaciones
        ha desaparecido: el motor de inferencia opera exclusivamente con
        \code{DiscreteTable} y, en el caso de análisis coste-efectividad,
        con \code{GenericTable<CEP>}.
\end{itemize}

\subsection{Capa 3: Computación}
\label{sec:ideal-capa3}

La capa de computación contiene las operaciones de inferencia sobre tablas
discretas.  Estas operaciones son métodos estáticos puros: reciben tablas,
devuelven tablas, y no tienen estado interno.

\begin{lstlisting}[caption={Operaciones de inferencia como funciones estáticas}]
public final class TableOperations {
    public static DiscreteTable multiply(List<DiscreteTable> potentials) { ... }
    public static DiscreteTable marginalize(DiscreteTable p, Variable var) { ... }
    public static DiscreteTable multiplyAndMarginalize(
            Collection<DiscreteTable> potentials,
            List<Variable> variablesToKeep,
            List<Variable> variablesToEliminate) { ... }
    public static DiscreteTable normalize(DiscreteTable p) { ... }
    public static DiscreteTable divide(DiscreteTable a, DiscreteTable b) { ... }
    public static DiscreteTable sum(List<DiscreteTable> potentials) { ... }
}
\end{lstlisting}
...
\begin{itemize}
  \item Las operaciones reciben y devuelven \code{DiscreteTable}, no
        \code{Potential}.  Esto elimina los \emph{downcasts}: si el compilador
        acepta la llamada, el tipo es correcto (requisito~\ref{req:rnf5}).

  \item \code{DiscreteTable} es una clase \code{final} con acceso directo
        al array \code{values} para las operaciones internas (paquete
        \code{operation}).  El acceso público es a través de \code{getValues()},
        pero los métodos del mismo paquete acceden al campo directamente
        para evitar la penalización de copias defensivas en el camino
        crítico (requisito~\ref{req:rnf1}).

  \item Las operaciones de coste-efectividad (\code{CEAlgebra}) operan
        sobre \code{GenericTable<CEP>} y usan \code{DiscreteTable} solo
        como tabla de probabilidades de entrada.  No hay herencia compartida
        entre \code{DiscreteTable} y \code{GenericTable<E>} más allá de
        la clase abstracta \code{IndexedTable} que encapsula la lógica de
        indexación (dimensiones, offsets, posición).

  \item La normalización delega en el objeto \code{PotentialSemantics}
        asociado a la tabla:
\begin{lstlisting}
public static DiscreteTable normalize(DiscreteTable table) {
    double[] result = table.getValues().clone();
    table.getSemantics().normalize(result, table.getDimensions(),
                                   table.getVariables());
    return DiscreteTable.of(table.getVariables(), result,
                            table.getSemantics());
}
\end{lstlisting}
        Esto elimina el \code{switch} sobre \code{PotentialRole}: cada
        semántica sabe cómo normalizarse.
\end{itemize}

% ===================================================================
\section{Rol como comportamiento: \code{PotentialSemantics}}
\label{sec:ideal-semantics}

El diseño actual usa el enum \code{PotentialRole} como \emph{type code}:
un campo que controla el comportamiento mediante sentencias \code{switch}
dispersas por todo el código (problema P6).  El diseño ideal convierte
cada rol en un \emph{objeto de comportamiento} que implementa una interfaz
común.  La figura~\ref{fig:diseno-semantics} muestra la jerarquía de
semánticas con el contraste entre el diseño anterior (enum + \code{switch})
y el nuevo (polimorfismo).

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{fig-diseno-semantics}
\caption{Jerarquía de \code{PotentialSemantics}.  Cada rol del antiguo enum
         \code{PotentialRole} se convierte en un objeto de comportamiento
         con sus propios métodos de inicialización y normalización.  Los
         recuadros rojo y verde contrastan el código \emph{antes} (sentencias
         \code{switch} sobre el enum) con el código \emph{después} (llamada
         polimórfica al objeto de semántica).}
\label{fig:diseno-semantics}
\end{figure}

\begin{lstlisting}[caption={Interfaz \code{PotentialSemantics} y sus implementaciones}]
public sealed interface PotentialSemantics
    permits ConditionalProbability, JointProbability,
            UtilitySemantics, PolicySemantics, RestrictionSemantics {

    void initializeUniform(double[] values, List<Variable> variables);
    void normalize(double[] values, int[] dimensions, List<Variable> variables);
    boolean canInduceFindings();
    boolean isAdditive();
}

// Singleton: la semantica no tiene estado (excepto UtilitySemantics)
public final class ConditionalProbability implements PotentialSemantics {
    public static final ConditionalProbability INSTANCE = new ConditionalProbability();

    @Override public void initializeUniform(double[] values, List<Variable> vars) {
        double v = 1.0 / vars.get(0).getNumStates();
        Arrays.fill(values, v);
    }
    @Override public void normalize(double[] values, int[] dims, List<Variable> vars) {
        // normaliza por rodajas del primer variable
        int sliceSize = dims[0];
        for (int offset = 0; offset < values.length; offset += sliceSize) {
            double sum = 0;
            for (int i = 0; i < sliceSize; i++) sum += values[offset + i];
            if (sum > 0) for (int i = 0; i < sliceSize; i++) values[offset + i] /= sum;
        }
    }
    @Override public boolean canInduceFindings() { return true; }
    @Override public boolean isAdditive()        { return false; }
}

public final class UtilitySemantics implements PotentialSemantics {
    private final Criterion criterion;  // lleva el criterio asociado

    @Override public void initializeUniform(double[] values, List<Variable> vars) {
        Arrays.fill(values, 0.0);
    }
    @Override public void normalize(double[] values, int[] dims, List<Variable> vars) {
        // no-op: las utilidades no se normalizan
    }
    @Override public boolean canInduceFindings() { return false; }
    @Override public boolean isAdditive()        { return true; }
}
\end{lstlisting}

\noindent \textbf{Ventajas sobre el enum \code{PotentialRole}:}

\begin{itemize}
  \item Cada \code{switch} actual se convierte en una llamada polimórfica:
        \code{table.getSemantics().normalize(values, dims, vars)}.  Si se
        añade un nuevo rol (p.~ej.\ \code{RewardSemantics} para MDPs), basta
        con crear una nueva implementación de \code{PotentialSemantics};
        no es necesario rastrear y actualizar sentencias \code{switch}
        dispersas (principio Abierto/Cerrado).

  \item \code{UtilitySemantics} lleva el \code{Criterion} como campo
        propio, eliminando el patrón actual donde la identidad de utilidad
        se codifica como \code{criterion != null} y el rol se guarda
        aparte como \code{PotentialRole.UNSPECIFIED}.

  \item Las implementaciones de probabilidad (\code{ConditionalProbability},
        \code{JointProbability}) y política son singletons sin estado,
        sin coste de memoria por instancia.

  \item El \emph{pattern matching} exhaustivo de Java~21 sobre la
        \code{sealed interface} garantiza que cada nuevo rol se maneje
        en todos los puntos del código que hacen distinción semántica.
\end{itemize}

% ===================================================================
\section{Metadatos por composición}
\label{sec:ideal-metadatos}

En el diseño actual, los metadatos de inferencia (árboles de estrategia y
distribuciones de incertidumbre) están integrados en \code{TablePotential}
como campos opcionales (problema~P1) o como subclases
(\code{StrategicTablePotential}, \code{UncertainTablePotential} tras el
Rediseño~1).

El diseño ideal reconoce que estos metadatos son \textbf{ortogonales} a la
tabla de valores y al rol semántico.  No son propiedades del potencial en
sí, sino \emph{anotaciones del resultado de la inferencia}:

\begin{itemize}
  \item Los \textbf{árboles de estrategia} solo aparecen como resultado de
        la maximización en la resolución de diagramas de influencia.
        Son generados por \code{MaxOutVariable} o \code{CEAlgebra.ceMaximize}
        y acompañan al potencial de utilidad resultante.

  \item Los \textbf{valores de incertidumbre} solo aparecen en potenciales
        cargados desde fichero para análisis de sensibilidad paramétrica.
        Son consumidos por el muestreo PSA (\emph{Probabilistic Sensitivity
        Analysis}) y no participan en la inferencia exacta.
\end{itemize}

En ambos casos, el array \code{double[] values} y su indexación no cambian.
Lo que cambia es el \emph{contexto} que acompaña a esos valores.  Por tanto,
el diseño ideal modela estos metadatos como contenedores externos que
\emph{componen} una tabla, no la extienden:

\begin{lstlisting}[caption={Metadatos como composición externa}]
// Contenedor para resultados de inferencia con estrategias
public final class InferenceAnnotation {
    private final DiscreteTable table;
    private final StrategyTree[] strategies;  // puede ser null
    private final UncertainValue[] uncertainty; // puede ser null

    public DiscreteTable getTable()             { return table; }
    public boolean hasStrategies()              { return strategies != null; }
    public StrategyTree[] getStrategies()       { return strategies; }
    public boolean hasUncertainty()             { return uncertainty != null; }
    public UncertainValue[] getUncertainValues(){ return uncertainty; }
}
\end{lstlisting}
...
\begin{itemize}
  \item \code{DiscreteTable} queda libre de campos opcionales.  No necesita
        subclases especializadas para estrategias o incertidumbre.  Es una
        clase \code{final} simple y predecible.

  \item La dependencia cíclica \code{TablePotential} $\to$ \code{StrategyTree}
        $\to$ \code{TreeADDPotential} $\to$ \code{TablePotential} (problema~P5)
        desaparece: \code{DiscreteTable} no referencia \code{StrategyTree};
        es el contenedor \code{InferenceAnnotation} quien lo hace, y ese
        contenedor vive en el módulo de inferencia, no en el de
        representación.

  \item El constructor de copia de \code{DiscreteTable} no tiene campos
        opcionales que copiar incorrectamente (problema~P1 eliminado
        por diseño).

  \item Las operaciones de inferencia que propagan estrategias (como
        \code{multiplyAndMarginalize} con \code{StrategicTablePotential})
        operan sobre \code{InferenceAnnotation} y su tabla interna, no
        sobre subclases de \code{DiscreteTable}.
\end{itemize}

% ===================================================================
\section{Eliminación de los \emph{downcasts}}
\label{sec:ideal-downcasts}

El problema~P4 (downcasts desprotegidos en inferencia) se resuelve
naturalmente con la arquitectura de tres capas:

\begin{enumerate}
  \item \textbf{Antes de la proyección:} los potenciales son polimorfos
        (\code{Potential}), pero el único código que los toca es el
        \code{ProjectionContext}, que despacha mediante \code{instanceof
        Projectable} o \emph{pattern matching} sobre la \code{sealed interface}.
        No hay \emph{downcasts} manuales.

  \item \textbf{Después de la proyección:} todo es \code{DiscreteTable} (o
        \code{GenericTable<CEP>} en el camino de coste-efectividad).  El
        módulo de inferencia (\code{VariableEliminationCore},
        \code{ChanceVariableElimination}, \code{HuginPropagation}) trabaja
        directamente con estos tipos concretos.  No necesita \emph{downcasts}
        porque nunca ve un \code{Potential} genérico.

  \item \textbf{El bifurcación unicriterio/bicriterio} en el motor de
        eliminación de variables se expresa con una clase sellada para
        el tipo de utilidad en juego:
\begin{lstlisting}
sealed interface UtilityResult permits DiscreteUtility, CEPUtility { }
record DiscreteUtility(DiscreteTable table) implements UtilityResult { }
record CEPUtility(GenericTable<CEP> table) implements UtilityResult { }
\end{lstlisting}
        El \emph{pattern matching} reemplaza los \code{instanceof GTablePotential}
        dispersos por el código actual.
\end{enumerate}

% ===================================================================
\section{Resolución de la dependencia cíclica}
\label{sec:ideal-ciclo}

El ciclo actual es:

\begin{center}
\code{TablePotential} $\longrightarrow$ \code{StrategyTree[]}
$\longrightarrow$ \code{TreeADDPotential}
$\longrightarrow$ \code{TablePotential}
\end{center}

En el diseño ideal, el ciclo se rompe en dos puntos:

\begin{enumerate}
  \item \code{DiscreteTable} no contiene \code{StrategyTree[]}.  Los
        árboles de estrategia viven en \code{InferenceAnnotation}, que
        está en el módulo de inferencia (no en \code{core}).

  \item \code{TreeADDPotential} no referencia \code{DiscreteTable}
        directamente.  Sus hojas implementan \code{Projectable}:
        la proyección de un \code{TreeADDPotential} navega el árbol y
        proyecta la hoja seleccionada, que devuelve una \code{DiscreteTable}.
        La relación es unidireccional.
\end{enumerate}

El grafo de dependencias resultante es un DAG:

\begin{center}
\code{InferenceAnnotation} $\to$ \code{DiscreteTable} \\
\code{InferenceAnnotation} $\to$ \code{StrategyTree} \\
\code{TreeADDPotential} $\to$ \code{Projectable} (interfaz)
\end{center}

No hay ciclos.  Cada componente puede compilarse, testarse y refactorizarse
de forma independiente.

% ===================================================================
\section{\code{GenericTable<E>}: posición limpia en la jerarquía}
\label{sec:ideal-gtable}

El diseño actual tiene un problema histórico con \code{GTablePotential<E>}
(problema~P3).  En el diseño ideal, la relación es clara:

\begin{lstlisting}[caption={Jerarquía limpia con \code{IndexedTable} como base}]
// Base comun: logica de indexacion (dimensiones, offsets, posicion)
public abstract sealed class IndexedTable
    permits DiscreteTable, GenericTable {
...
// Tabla de valores double: el caballo de batalla de la inferencia
public final class DiscreteTable extends IndexedTable
    implements Potential, Projectable, Reorderable, Scalable { ... }

// Tabla de objetos genericos: para coste-efectividad (CEP)
public final class GenericTable<E> extends IndexedTable {
    private final List<E> elementTable;
}
\end{lstlisting}

\code{GenericTable<E>} ya no hereda de \code{DiscreteTable}: no tiene
un array \code{values[]} inútil, no pasa \code{null} como rol, y no
hereda métodos (\code{setUniform()}, \code{normalize()}) que no tienen
sentido para una tabla de objetos.  La única lógica compartida ---la
indexación--- está correctamente factorizada en \code{IndexedTable}.

% ===================================================================
\section{Evaluación contra los problemas originales}
\label{sec:ideal-evaluacion}

La tabla~\ref{tab:ideal-evaluacion} resume cómo el diseño ideal resuelve
cada uno de los seis problemas identificados en el catálogo
(sección~\ref{sec:catalogo}).
...
\toprule
\textbf{Id} & \textbf{Problema} & \textbf{Cómo lo resuelve el diseño ideal} & \textbf{Estado} \\
\midrule
\rowcolor{lightgray}
P1 & God Class &
  \code{DiscreteTable} es \code{final}, sin campos opcionales.
  Estrategias e incertidumbre se componen externamente en
  \code{InferenceAnnotation}. &
  \good{Resuelto} \\

P2 & Métodos forzados &
  \code{Potential} es una \code{sealed interface} sin métodos abstractos
  de operación.  Cada tipo implementa solo las interfaces de capacidad
  que soporta. &
  \good{Resuelto} \\

\rowcolor{lightgray}
P3 & GTablePotential &
  \code{GenericTable<E>} hereda de \code{IndexedTable}, no de
  \code{DiscreteTable}.  No tiene \code{values[]} ni \code{PotentialRole}. &
  \good{Resuelto} \\

P4 & Downcasts &
  Después de la proyección, todo es \code{DiscreteTable}.  La inferencia
  no necesita \emph{downcasts}. &
  \good{Resuelto} \\

\rowcolor{lightgray}
P5 & Dependencia cíclica &
  \code{DiscreteTable} no contiene \code{StrategyTree[]}.
  \code{TreeADDPotential} referencia \code{Projectable}, no
  \code{DiscreteTable}. &
  \good{Resuelto} \\

P6 & Type code &
  \code{PotentialSemantics} encapsula el comportamiento de cada rol.
  No hay sentencias \code{switch} sobre un enum. &
  \good{Resuelto} \\
\bottomrule
\end{tabular}
\caption{Evaluación del diseño ideal contra los seis problemas del catálogo.}
\label{tab:ideal-evaluacion}
\end{table}

% ===================================================================
\section{Evaluación contra los requisitos}
\label{sec:ideal-eval-req}
...
\toprule
\textbf{Req.} & \textbf{Estado} & \textbf{Cómo se satisface} \\
\midrule
\rowcolor{lightgray}
RF1 & \good{Sí} & La \code{sealed interface Potential} admite tantos
                    \code{permits} como representaciones se necesiten. \\
RF2 & \good{Sí} & Cada representación implementa \code{Projectable};
                    el \code{ProjectionContext} coordina la fase. \\
\rowcolor{lightgray}
RF3 & \good{Sí} & \code{TableOperations} contiene todas las operaciones
                    de inferencia sobre \code{DiscreteTable}. \\
RF4 & \good{Sí} & \code{PotentialSemantics} encapsula el comportamiento
                    de cada rol con polimorfismo. \\
\rowcolor{lightgray}
RF5 & \good{Sí} & \code{GenericTable<E>} comparte \code{IndexedTable}
                    con \code{DiscreteTable}; \code{CEAlgebra} opera
                    sobre \code{GenericTable<CEP>}. \\
RF6 & \good{Sí} & \code{InferenceAnnotation} compone tabla + estrategias
                    + incertidumbre sin herencia. \\
\rowcolor{lightgray}
RF7 & \good{Sí} & \code{ProjectionContext} provee las tablas ya proyectadas
                    para \code{SameAsPrevious} y similares. \\
\midrule
RNF1 & \good{Sí} & \code{DiscreteTable.values} accesible directamente
                     desde \code{TableOperations} (mismo paquete). \\
\rowcolor{lightgray}
RNF2 & \good{Sí} & Nuevo potencial = nueva clase \code{implements
                     Projectable}; nada más cambia. \\
RNF3 & \good{Sí} & Nueva operación = nuevo método en
                     \code{TableOperations}; la jerarquía no cambia. \\
\rowcolor{lightgray}
RNF4 & \good{Sí} & Sin ciclos; cada capa testable por separado. \\
RNF5 & \good{Sí} & \code{sealed interface} + interfaces de capacidad
                     impiden errores de tipo en compilación. \\
\bottomrule
\end{tabular}
\caption{Evaluación del diseño ideal contra los requisitos funcionales y
         no funcionales.}
\label{tab:ideal-eval-req}
\end{table}

% ===================================================================
\section{Impacto en rendimiento}
\label{sec:ideal-rendimiento}

\textbf{Tiempo}: El diseño ideal no introduce ninguna penalización en el
camino crítico de la inferencia.  Las operaciones de \code{TableOperations}
acceden directamente al array \code{values[]} de \code{DiscreteTable} (por
visibilidad de paquete), sin copias defensivas ni indirecciones virtuales
adicionales respecto al diseño actual.  El despacho polimórfico solo ocurre
en la fase de proyección (una vez por potencial), no en la fase de computación
(miles de veces por variable eliminada).

La sustitución de los \code{switch} sobre \code{PotentialRole} por llamadas
polimórficas a \code{PotentialSemantics} tiene un coste despreciable: la JVM
moderna resuelve estas llamadas con un despacho monomorfo en los puntos
calientes (la semántica concreta es casi siempre la misma dentro de un
contexto de inferencia dado).

\textbf{Memoria}: La eliminación de los campos opcionales en
\code{DiscreteTable} ahorra 2 punteros ($\approx$16~bytes en JVM de 64~bits
con \emph{compressed oops}) por instancia respecto al \code{TablePotential}
original.  \code{GenericTable<E>} no hereda un \code{double[]} inútil,
ahorrando \code{tableSize * 8} bytes por instancia de tabla genérica.
\code{PotentialSemantics} añade un puntero por instancia de tabla ($\approx$8
bytes), pero las semánticas de probabilidad, política y restricción son
singletons sin coste adicional.

En conjunto, el impacto neto sobre memoria es una \good{reducción moderada},
coherente con la Opción~1 y sin las penalizaciones de la Opción~5
(inmutabilidad).

% ===================================================================
\section{Distancia al diseño actual tras los rediseños 1 y 3}
\label{sec:ideal-distancia}

Tras la implementación de los Rediseños~1 y~3, el diseño actual ya ha
recorrido una parte significativa del camino hacia el diseño ideal:
...
\begin{tabular}{lcc}
\toprule
\textbf{Aspecto} & \textbf{Implementado} & \textbf{Pendiente} \\
\midrule
\rowcolor{lightgray}
Extracción de campos opcionales a subclases & \good{Sí} (Rediseño~1) & --- \\
Interfaces de capacidad (\code{Projectable}, etc.) & \good{Sí} (Rediseño~3) & --- \\
\rowcolor{lightgray}
\code{AbstractIndexedPotential} como base de indexación & \good{Sí} (Rediseño~1) & Renombrar a \code{IndexedTable} \\
Encapsulación de \code{values} & \good{Sí} & --- \\
\rowcolor{lightgray}
\code{GTablePotential} desacoplado de \code{TablePotential} & \warn{Parcial} & Cambiar superclase (fase~5) \\
\code{Potential} como \code{sealed interface} & \bad{No} & Migración de clase abstracta a interfaz \\
\rowcolor{lightgray}
\code{PotentialSemantics} en vez de \code{PotentialRole} enum & \bad{No} & Crear jerarquía de semánticas \\
Metadatos como composición (\code{InferenceAnnotation}) & \bad{No} & Extraer de subclases a contenedor \\
\rowcolor{lightgray}
Eliminación de \emph{downcasts} en inferencia & \bad{No} & Requiere refactorizar módulo inference \\
\code{UtilityResult} sellado para bifurcación uni/bicriterio & \bad{No} & Requiere refactorizar \code{VariableEliminationCore} \\
\bottomrule
\end{tabular}
\caption{Distancia entre el diseño actual (tras Rediseños~1 y~3) y el diseño
         ideal.  Los aspectos ya implementados son la base sobre la que
         construir las fases restantes.}
\label{tab:ideal-distancia}
\end{table}

\subsection{Hoja de ruta sugerida}

La transición del diseño actual al ideal puede hacerse en fases incrementales,
cada una de las cuales deja el sistema en un estado funcional:

\begin{enumerate}
  \item \textbf{Fase A: Cambio de superclase de \code{GTablePotential}}
        (ya planificada como fase~5 del Rediseño~1).  \code{GTablePotential}
        pasa a extender \code{AbstractIndexedPotential} en lugar de
        \code{TablePotential}.  Elimina el \code{values[]} inútil.

  \item \textbf{Fase B: Introducir \code{PotentialSemantics}}.
        Crear las implementaciones de \code{PotentialSemantics}
        (\code{ConditionalProbability}, \code{JointProbability}, etc.) y
        migrar los \code{switch} en \code{TablePotential} y
        \code{DiscretePotentialOperations} a llamadas polimórficas.
        \code{PotentialRole} se mantiene temporalmente como campo pero
        se delega en \code{PotentialSemantics} para el comportamiento.

  \item \textbf{Fase C: Metadatos como composición}.
        Introducir \code{InferenceAnnotation} en el módulo de inferencia.
        Los algoritmos de maximización (\code{MaxOutVariable}) producen
        \code{InferenceAnnotation} en vez de \code{StrategicTablePotential}.
        \code{StrategicTablePotential} y \code{UncertainTablePotential}
        pueden mantenerse para compatibilidad en E/S, pero la inferencia
        ya no los usa internamente.

  \item \textbf{Fase D: Tipado fuerte en inferencia}.
        Refactorizar \code{VariableEliminationCore},
        \code{ChanceVariableElimination} y \code{DecisionVariableElimination}
        para que trabajen con \code{DiscreteTable} en vez de
        \code{TablePotential}.  Introducir \code{UtilityResult} sellado.
        Eliminar \emph{downcasts}.

  \item \textbf{Fase E: \code{Potential} como \code{sealed interface}}.
        Migrar la clase abstracta \code{Potential} a una \code{sealed
        interface}.  Requiere Java~17+ y es la fase de mayor impacto en
        la API pública, pero a estas alturas la mayoría del código ya
        trabaja con \code{DiscreteTable} directamente.
\end{enumerate}

Cada fase es independiente y aporta valor por sí misma: la Fase~A limpia
\code{GTablePotential}, la Fase~B elimina los \code{switch}, la Fase~C
rompe el ciclo de dependencias, la Fase~D elimina los \emph{downcasts},
y la Fase~E completa la seguridad de tipos.  No es necesario completarlas
todas para obtener beneficios.
